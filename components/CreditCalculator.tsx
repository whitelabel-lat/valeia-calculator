...
export default function CreditCalculator() {
  const [planSeleccionado, setPlanSeleccionado] = useState<keyof typeof PLANES>('emprendedor');
  ...
  const maxMensajesPosibles = Math.floor(PLANES[planSeleccionado].creditos / COSTO_MENSAJE);
  const maxMinutosPosibles = Math.floor(PLANES[planSeleccionado].creditos / COSTO_LLAMADA);
      ...
      <CardContent className="space-y-6">
        {resultados && resultados.creditosRestantes < 0 && (
          <Alert variant="destructive" className="mb-6">
            <AlertDescription>
              Has excedido la cantidad de créditos de tu plan por {Math.abs(resultados.creditosRestantes)} créditos. 
              Considera adquirir más créditos o cambiar a un plan superior.
            </AlertDescription>
          </Alert>
        )}
            ...
            <div className="relative mx-auto max-w-xl">
              <div className="flex rounded-full bg-gradient-to-r from-[#40C9FF] via-[#6666FF] to-[#9933FF] p-[1px]">
                {Object.entries(PLANES).map(([key, plan], index) => (
                  <button
                    key={key}
                    onClick={() => setPlanSeleccionado(key as keyof typeof PLANES)}
                    className={cn(
                      ...
                      index === Object.entries(PLANES).length - 1 && "rounded-r-full",
                      planSeleccionado === key && `bg-gradient-to-r ${plan.color}`
                    )}
                  >
                    {plan.nombre}
                  </button>
            ...
            <div className="text-center space-y-4">
              <div className="space-y-1">
                <div className="text-4xl font-bold flex items-center justify-center gap-1">
                  ...
                  <span className="text-base text-gray-500 font-normal">USD/month</span>
                </div>
              ...
              <div className="pt-4 flex gap-4 justify-center">
                <Button 
                  className="bg-[#6666FF] hover:bg-[#5151FF]"
                >
                  Suscríbete
                </Button>
                <Button 
          ...
          <div className="mt-8 p-4 bg-gray-50 rounded-lg">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{resultados.creditosIniciales}</span>
              </div>
              <div className="flex justify-between">
                <span>Total consumido:</span>
                <span className="font-medium">{resultados.totalConsumido}</span>
              </div>
              <div className="flex justify-between border-t pt-2">
                <span>Créditos restantes:</span>
                <span className={`font-medium ${resultados.creditosRestantes < 0 ? 'text-red-600' : ''}`}>
